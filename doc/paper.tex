...
Using this environment, we explore three different ways of training the agent:

\subsubsection{Standard Proximal Policy Optimization}

A standard PPO algorithm with no LLM integration, as a baseline. In this case, we StableBaselines' implementation with the following hyperparamers: HYPERPARAMETERS HERE.

\subsubsection{PPO with LLM integration as part of the policy}

In this case, the LLM is integrated as part of the PPO algorithm, directly influencing the actions taken. This is done by INSERT HOW IT IS DONE HERE.

\subsubsection{LLM integrated into reward function}

LLM integration into the reward function, where actions similar to those suggested by the LLM grant a small auxilliary reward. The agent is trained as in the baseline.


We evaluate these models on the same Minigrid environments and compare their performance against eachother.
