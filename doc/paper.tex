...
We also notice a significant performance increase when using LLM policy optimization, even somewhat more than from reward shaping. Averaging over all episodes of all runs, we observe a NUMBER\% increase in reward per episode compared to the PPO baseline, and a NUMBER\% increase when compared to LLM policy shaping. The standard deviation when using LLM policy optimization is also similarly large to PPO and reward shaping (NUMBER). By caching LLM responses similarly to when performing reward shaping, this method too adds very little computational cost to the training in total.

***ADD PARAGRAPH ABOUT USING BOTH TOGETHER*** However, the results from both of these methods are eclipsed by the results from combining LLM reward shaping and policy optimization together. Averaging over 8 runs, we observe a NUMBER\% increase in reward from the environment compared to PPO alone, NUMBER\% higher than reward shaping and NUMBER\%higher than policy optimization, with a similar standard deviation of NUMBER. 
...
Compared to the baseline PPO when averaged per episode over all 8 runs, LLM reward shaping gave a NUMBER\% increase in reward from the environment, with a standard deviation of NUMBER (PPO alone had a standard deviation of NUMBER). LLM policy optimization gave a NUMBER\% increase compared to the PPO baseline and a NUMBER\% increase compared to reward shaping. Also here giving the highest average reward per episode, the combination of reward shaping and policy optimization gave NUMBER\% more reward compared to the PPO baseline, NUMBER\% more than reward shaping and NUMBER\% higher than policy optimization.

\section{Conclusion}

It works man, dogski did it!

As shown in the previously referenced work (\ref{background}), the integration of a Large Language Model into the Reinforcement Learning framework can improve network performance. In the Minigrid environments we have tested on, DoorKey and Pickup Unlock, we have demonstrated so. This has been done both through the LLM shaping the RL reward function, and through the LLM influencing the action probabilities of the RL agent, as well as through combining these two methodologies.

For the DoorKey environment, we found that all three methods surpassed the performance of our PPO baseline. 
