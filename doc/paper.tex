...
% LLM policy optimization gave a NUMBER\% increase compared to the PPO baseline and a NUMBER\% increase compared to reward shaping. 

\section{Conclusion}

As shown in the previously referenced work (\ref{background}), the integration of a Large Language Model into the Reinforcement Learning framework can improve model performance. In the Minigrid environments we have tested on, we have shown that LLM reward shaping gives increased performance across the board, which shows it to be a good candidate for reward shaping in many environments with sparse rewards. Our method of policy integration has not shown similar increases, giving lower rewards on average than the baseline.

For the DoorKey environment, we found that LLM reward shaping gives a very significant $24\%$ increase in average rewards, aiding the agent in all stages of training. LLM policy influencing has sadly not given the same increase, underperforming in the more difficult environments.

For the UnlockPickup environment, we find that all methods struggle to complete the goal, but that LLM-aided methods also here outperform vanilla PPO, achieving METRIC.
